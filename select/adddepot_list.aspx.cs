...
public partial class select_adddepot_list : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            {
                RptBind("product_code_state='入库' " + CombSqlTxt(this.product_category_id, this.note_no, this.start_time, this.stop_time), "add_time desc,id desc");
            }
    ...
    private void RptBind(string _strWhere, string _orderby)
    {
        ...
        string pageUrl = Utils.CombUrlTxt("adddepot_list.aspx", "start_time={0}&stop_time={1}&product_category_id={2}&note_no={3}&page={4}", this.start_time.ToString(), this.stop_time.ToString(), this.product_category_id.ToString(),this.note_no, "__id__");
    ...
    {
        int _pagesize;
        if (int.TryParse(Utils.GetCookie("adddepot_page_size"), out _pagesize))
        {
            if (_pagesize > 0)
            {
                return _pagesize;
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("adddepot_list.aspx", "start_time={0}&stop_time={1}&product_category_id={2}&note_no={3}", this.txtstart_time.Value, this.txtstop_time.Value, this.product_category_id.ToString(), txtNote_no.Text)); 
    }
    
    //筛选商品类别
    protected void ddlproduct_category_id_SelectedIndexChanged(object sender, EventArgs e)
    {
        Response.Redirect(Utils.CombUrlTxt("adddepot_list.aspx", "start_time={0}&stop_time={1}&product_category_id={2}&note_no={3}", this.txtstart_time.Value, this.txtstop_time.Value, this.ddlproduct_category_id.SelectedValue, txtNote_no.Text)); 
    ...
    protected void txtPageNum_TextChanged(object sender, EventArgs e)
    {
        ...
        if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
        {
            if (_pagesize > 0)
            {
                Utils.WriteCookie("adddepot_page_size", _pagesize.ToString(), 14400);
            }
        }
        Response.Redirect(Utils.CombUrlTxt("adddepot_list.aspx", "start_time={0}&stop_time={1}&product_category_id={2}&note_no={3}", this.txtstart_time.Value, this.txtstop_time.Value, this.product_category_id.ToString(), txtNote_no.Text)); 
    }
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("adddepot_rep.aspx", "start_time={0}&stop_time={1}&product_category_id={2}&note_no={3}", this.txtstart_time.Value, this.txtstop_time.Value, this.product_category_id.ToString(), txtNote_no.Text)); 
    }
